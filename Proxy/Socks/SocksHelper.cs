...
internal static class SocksHelper
{
    ...
    private static async ValueTask EstablishSocks5TunnelAsync(Stream stream, string host, int port, NetworkCredential? credentials, bool async)
    {
        ...
        try
        {
            ...
            switch (buffer[1])
            {
                ...
                case METHOD_USERNAME_PASSWORD:
                {
                    ...
                    if (credentials is null)
                    {
                        ...
                        throw new SocksException("net_socks_auth_required");
                    }
                    ...
                    {
                        throw new SocksException("net_socks_auth_failed");
                    }
                    break;
                }

                default:
                    throw new SocksException("net_socks_no_auth_method");
            }
            ...
            {
                throw new SocksException("net_socks_connection_failed");
            }
            int bytesToSkip = buffer[3] switch
            {
                ...
                _ => throw new SocksException("net_socks_bad_address_type")
            };
    ...
    private static async ValueTask EstablishSocks4TunnelAsync(Stream stream, bool isVersion4a, string host, int port, NetworkCredential? credentials, bool async, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (IPAddress.TryParse(host, out IPAddress? hostIP))
            {
                ...
                {
                    throw new SocksException("net_socks_ipv6_notsupported");
                }
            }
            else if (!isVersion4a)
            {
                ...
                {
                    throw new SocksException("net_socks_no_ipv4_address", ex);
                }

                if (addresses.Length == 0)
                {
                    throw new SocksException("net_socks_no_ipv4_address");
                }
    ...
    private static byte EncodeString(ReadOnlySpan<char> chars, Span<byte> buffer, string parameterName)
    {
        ...
        catch
        {
            ...
            throw new SocksException("net_socks_string_too_long");
        }
    }
    ...
    private static void VerifyProtocolVersion(byte expected, byte version)
    {
        if (expected != version)
        {
            throw new SocksException("net_socks_unexpected_version");
        }
    }
